...
class KhsManagementService
{
    ...
    public function uploadKhsForStudent(
        ...
        // 3) Target folder (boleh kosong = root)
        ...
        $created = $service->files->create($driveFile, [
            ...
            // 'supportsAllDrives' => true, // aktifkan jika pakai Shared Drive
        ]);

        $fileId       = $created->id;
        $webViewLink  = $created->webViewLink ?: "https://drive.google.com/file/d/{$fileId}/view";
        $downloadLink = $created->webContentLink ?: "https://drive.google.com/uc?id={$fileId}&export=download";

        // 5) Buka akses publik baca (opsional, agar orang tua bisa akses link)
        try {
        ...
        // 6) Simpan DB (sesuaikan field model KhsFile kamu)
        return KhsFile::create([
            'student_id'         => $student->id,
            'academic_period_id' => $period->id,
            'file_name'          => $finalName,
            'mime_type'          => $file->getClientMimeType(),
            'file_size'          => $file->getSize(),
            'storage_disk'       => 'google-api',
            'storage_path'       => $fileId,
            'drive_file_id'      => $fileId,
            'uploaded_by'        => $uploaderId,
            'is_active'          => true,
            // kalau punya kolom link:
